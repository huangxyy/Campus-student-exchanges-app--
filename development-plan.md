...
- [x] 阶段 1（Week 1）：项目骨架 + 登录页 + 首页 + 商品列表（基础查询）
- [x] 阶段 2（Week 2）：商品发布、商品详情、我的商品、搜索筛选完善
- [x] 阶段 3（Week 3）：任务大厅、任务发布、会话列表、聊天详情、实时与风控收口
- [x] 阶段 4（Week 4）：求购广场 + 到货提醒 + 订单系统（双确认 + 评价）
- [x] 阶段 5（Week 5）：校园动态 + 积分体系（等级、兑换、排行榜多维度）+ 举报流程
- [x] 阶段 6（Week 6）：校园维基（评论区、投稿审核）+ 活动专题 + 个人中心增强
- [x] 阶段 7（Week 7）：性能优化（列表查询缓存、骨架屏）+ 信任体系完善（徽章展示）
- [ ] 阶段 8（Week 8）：全量测试 + 提审上线 + 发布收口（进行中：本地生产构建通过）
